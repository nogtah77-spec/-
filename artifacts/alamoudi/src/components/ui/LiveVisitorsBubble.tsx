...
export function LiveVisitorsBubble() {
  ...
  // Collapsed state persisted in localStorage: default false (expanded) or user's preference
  const [collapsed, setCollapsed] = useState<boolean>(() => {
    try { return localStorage.getItem(STATE_KEY) === "true"; } catch { return false; }
  });

  const currentH = collapsed ? H_COLLAPSED : H_EXPANDED;
  ...
  const [pos, setPos] = useState<Pos>(() => {
    try {
      if (typeof window !== "undefined") {
        ...
        if (raw) {
          return getSafePosition(JSON.parse(raw), BUBBLE_WIDTH, H_COLLAPSED);
        }
      }
    } catch {}
    const winH = typeof window !== "undefined" ? window.innerHeight : 800;
    return { x: MARGIN, y: Math.max(MARGIN + 60, winH - H_COLLAPSED - 90) };
  });
  ...
  useEffect(() => {
    try {
      const raw = localStorage.getItem(POS_KEY);
      if (raw) {
        setPos(getSafePosition(JSON.parse(raw), BUBBLE_WIDTH, currentH));
        return;
      }
    } catch {}
    const winH = typeof window !== "undefined" ? window.innerHeight : 800;
  ...
  useEffect(() => {
    const onResize = () => {
      if (posRef.current) setPos(clampToViewport(posRef.current.x, posRef.current.y, BUBBLE_WIDTH, currentH));
    };
  ...
  const onPointerMove = useCallback((e: React.PointerEvent) => {
    ...
    const np = clampToViewport(e.clientX - d.dx, e.clientY - d.dy, BUBBLE_WIDTH, currentH);
    setPos(np);
  }, [currentH]);
  ...
  const onPointerUp = useCallback(() => {
    ...
    // If tapped without dragging: toggle collapse smoothly (never navigate away!)
    if (!d.moved) {
      toggleCollapse();
    }
  }, [toggleCollapse]);
      ...
      <div className="rounded-2xl border border-amber-400/40 bg-gradient-to-br from-[#1e293b]/98 to-[#0f172a]/98 backdrop-blur-md shadow-2xl shadow-black/50 ring-1 ring-white/15 overflow-hidden transition-all duration-300">
        ...
        >
          ...
          <div className="flex min-w-0 flex-1 flex-col leading-none">
            <span className="text-[9.5px] font-bold text-amber-200/90 tracking-tight">متواجدون الآن</span>
            <span className="text-lg font-black text-white leading-tight drop-shadow-xs tabular-nums">
              <RollingNumber value={onlineCount} />
            </span>
          </div>

          {/* Explicit Minimize/Expand Toggle Button */}
          <button
            type="button"
            onPointerDown={e => e.stopPropagation()}
            onPointerUp={e => e.stopPropagation()}
            onClick={toggleCollapse}
            className="w-6 h-6 flex items-center justify-center text-amber-200/80 hover:text-white hover:bg-white/10 transition-colors rounded-md shrink-0"
            title={collapsed ? "توسيع وعرض تفاصيل الإحصائيات" : "تصغير إلى شريط ملخص"}
          >
            {collapsed ? (
        ...
        {/* Detailed Metrics Drawer (Expanded mode) */}
        {!collapsed && (
