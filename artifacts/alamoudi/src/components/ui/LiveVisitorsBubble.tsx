...
import { Radio, GripVertical, ChevronDown, ChevronUp, Users, Eye, CalendarDays, TrendingUp, Minimize2 } from "lucide-react";
...
export function LiveVisitorsBubble() {
  ...
  // Collapsed state: false = expanded with 4 stats, true = compact header bar
  ...
  const currentW = W_NORMAL;
  const currentH = collapsed ? H_BAR : H_DRAWER;
  ...
  // Toggle collapse: expands or shrinks the card in-place
  ...
  const handleMinimize = useCallback((e?: React.MouseEvent) => {
    if (e) e.stopPropagation();
    setCollapsed(true);
    try { localStorage.setItem(STATE_KEY, "true"); } catch {}
  }, []);
  ...
  const handleExpand = useCallback((e?: React.MouseEvent) => {
    if (e) e.stopPropagation();
    setCollapsed(false);
    try { localStorage.setItem(STATE_KEY, "false"); } catch {}
  }, []);
  ...
  const onPointerUp = useCallback(() => {
    ...
    // If tapped without dragging: toggle expand / minimize smoothly
    if (!d.moved) {
      toggleCollapse();
    }
  }, [toggleCollapse]);
    ...
    >
      <div className="rounded-2xl border-2 border-amber-400/60 bg-gradient-to-br from-[#10202D] via-[#173044] to-[#0D1B27] backdrop-blur-md shadow-2xl shadow-black/80 ring-1 ring-white/20 overflow-hidden transition-all duration-300">
        ...
        <div
          ...
          title={collapsed ? "متواجدون الآن - انقر للتوسيع أو اسحب للتحريك" : "متواجدون الآن - انقر للتصغير أو اسحب للتحريك"}
        >
          ...
          {/* Action buttons area */}
          {collapsed ? (
            /* Mode A: When Minimized/Collapsed -> Show Expand Buttons */
            <button
              type="button"
              onPointerDown={e => e.stopPropagation()}
              onPointerUp={e => e.stopPropagation()}
              onClick={handleExpand}
              className="flex items-center gap-1 px-1.5 py-1 text-amber-200/90 hover:text-white hover:bg-white/10 transition-colors rounded shrink-0 text-[10px] font-bold"
              title="توسيع النافذة وعرض تفاصيل الإحصائيات"
            >
              <ChevronDown className="h-4 w-4 transition-transform duration-200" />
            </button>
          ) : (
            /* Mode B: When Expanded -> Show Minimize & Curtain Buttons */
            <div className="flex items-center gap-0.5 shrink-0">
              {/* زر التصغير الصريح (Minimize) */}
              <button
                type="button"
                onPointerDown={e => e.stopPropagation()}
                onPointerUp={e => e.stopPropagation()}
                onClick={handleMinimize}
                className="w-6 h-6 flex items-center justify-center text-amber-200/90 hover:text-white hover:bg-white/10 transition-colors rounded"
                title="تصغير حجم النافذة (إغلاق التفاصيل)"
              >
                <Minimize2 className="h-3.5 w-3.5 hover:scale-110 transition-transform" />
              </button>

              {/* زر قفل الستارة (Curtain Chevron Up) */}
              <button
                type="button"
                onPointerDown={e => e.stopPropagation()}
                onPointerUp={e => e.stopPropagation()}
                ...
                title="قفل الستارة"
              >
                <ChevronUp className="h-4 w-4 hover:-translate-y-0.5 transition-transform" />
              </button>
            </div>
          )}
